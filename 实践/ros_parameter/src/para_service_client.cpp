...
int main(int argc, char **argv)                // 节点主函数
{
    ...
    */
    ros::init(argc, argv, "para_service_client"); // 解析参数，命名节点
    ...
    */
    ...
    if (argc != 3)                           // 处理输入值错误
    {
        ...
        */  
        // argc!=3,说明rosrun ros_parameter para_service_client 后面跟的参数个数不符合要求,需要提示输入两个数字 arg0 arg1
        ROS_INFO("cmd : rosrun ros_parameter para_service_client arg0 arg1");
    ...
    */
    ...
    ros::ServiceClient ros_para_service_client = nh.serviceClient<ros_parameter::ParaServiceTest>("ros_para_service_srv");
    /*
    ...
    */ 
    ...
    ros_parameter::ParaServiceTest srv;
    ...
    */
    ...
    // 请求服务，如果请求被接受，则显示响应值
    if (ros_para_service_client.call(srv))        // 服务服务器中有回调函数，输入参数req，输出参数res， bool 返回值由于标志是否处理成功
    {
    ...
    {
        ROS_ERROR("Failed to call service ros_para_service_srv");
        return 1;
    }
    /* 控制台输出。用来取代printf/cout。见文档：http://wiki.ros.org/rosconsole
    ROS_INFO和它的朋友们可用来取代printf/cout。
